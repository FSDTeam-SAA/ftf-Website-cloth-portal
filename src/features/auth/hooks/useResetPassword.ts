...
export function useResetPassword() {
    ...
    const handleResetPassword = async (data: { newPassword: string; confirmPassword: string }, accessToken: string) => {
        setLoading(true);
        setError(null);
        setSuccess(false);
        try {
            const response = await resetPasswordApi(data, accessToken);
            setSuccess(true);
            return response;
        } catch (err: unknown) {
            if (isAxiosError(err)) {
